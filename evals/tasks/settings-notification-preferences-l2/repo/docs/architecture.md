...
- `prefs/` holds the user settings code; `infra/` holds shared plumbing (configuration, logging, metrics, clock).
...
- The settings document is shared: the mobile sync worker writes `notifications.push` and the mailer writes `notifications.digest`; this service owns the rest.
