...
def funnel(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "due_rows": [i for i, (_, w) in enumerate(waited) if w >= DISCARD_AFTER],
...
def triage(apply: bool, root: Path = ROOT) -> list[str]:
    ...
    Rows are matched by position in the queue, never by slug: two rows may share one, and only the one that
    waited is due.
    """
    state = funnel(root)
    due = state["discard_due"]
    if not apply or not due:
        return due
    path = root / "meta/tracking/candidates.md"
    lines = path.read_text(encoding="utf-8").split("\n")
    at = next(i for i, line in enumerate(lines) if line.startswith(CANDIDATES_HEADER))
    rows = set(state["due_rows"])
    kept, gone, n = [], [], -1
    for i, line in enumerate(lines):
        if i >= at + 2 and line.startswith("|") and (n := n + 1) in rows and all(
                lines[j].startswith("|") for j in range(at + 2, i + 1)):
            cells = split_row(line)
            gone.append(_row([f"`{slug_of(cells[0])}`",
                              f"**discarded**: waited {DISCARD_AFTER} releases without gaining what it lacked ({cells[2]})"]))
            continue
        kept.append(line)
