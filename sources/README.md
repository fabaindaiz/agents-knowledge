...
**The queue keeps what it met.** `release.py intake` enters each new candidate with *Since*, the release it arrived at; a row offered again for a known slug lands in the queue's *Offered again, to merge* table, and the release adds it to its row or note as another occurrence; a row a harvest refused (`refused: <reason>` in *Lacks*) goes straight to the history. Nothing is dropped by age: a candidate leaves the queue only by a release's decision, written in [`../meta/tracking/history.md`](../meta/tracking/history.md) with its reason. The ledger, [`../meta/tracking/INDEX.md`](../meta/tracking/INDEX.md), generated by the build, lists every note under review or retired and every candidate queued or answered, one line each, so a meta-session extends what exists instead of creating it again.
