...
confidence: "reasoned"
phases: ["dataset", "tests"]
check: "a truncation test over the whole feature set, plus a planted leak that makes it fail"
about:
  - {do: "Add a feature to a point-in-time dataset", wrong_when: "the feature reads state without history, or its knowable-at timestamp is not its event timestamp"}
