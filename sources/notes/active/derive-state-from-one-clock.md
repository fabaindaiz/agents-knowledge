...
about:
  - {do: "Animate, schedule or simulate anything that must stay in step with a clock, or read a timing value from the platform", wrong_when: "state is advanced by adding each tick's delta, or a reported interval, end event or backward step is taken at its word"}
