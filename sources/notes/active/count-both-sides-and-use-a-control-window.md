...
about:
  - {do: "Investigate a discrepancy with a join, especially around an incident", wrong_when: "one side gained the join key later, so the gap is in the key and not in the world"}
rests_on: "difference-in-differences"
